...
function generateIntelligentFallback(
  ...
  if (match) {
    ...
    let dtcData = {
      ...
      causes: [
        "Corroded wiring harness pins",
        "Sensor out of calibration range",
    ...
    if (code.startsWith("P0300")) {
      ...
      dtcData = {
        ...
        causes: [
          "Degraded or damaged catalytic converter catalytic bed",
          "Exhaust manifold or pipe leak upstream of converter",
          "Engine misfiring or burning oil (poisoning substrate)",
        ],
        steps: [
      ...
      dtcData = {
        ...
        symptoms: [
          "Multiple indicator warnings lit",
          "No crank / No start condition",
          "Diagnostic tool unable to establish communication with ECU",
        ],
        causes: [
          "Blown main PCM relay or fuse",
          "Damaged CAN High or CAN Low bus wires",
          "Corroded ground strap from block to chassis",
          "Failed PCM unit",
        ],
        steps: [
...
*To activate live, real-time sync with Meli's global network, configure your valid **Meli Key** or authentication credentials in the **Settings** layout.*`;
  ...
  if (
    cleanMsg.includes("WIRING") ||
    cleanMsg.includes("DIAGRAM") ||
    cleanMsg.includes("SCHEMATIC") ||
    cleanMsg.includes("ALLDATA")
  ) {
...
async function startServer() {
  ...
  app.post("/api/git/sync", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Synchronized with remote repo.",
        output: stdout || stderr,
      });
    } catch (error: any) {
  ...
  app.post("/api/git/pull", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Remote repository updates pulled successfully.",
        output: stdout || stderr,
      });
    } catch (error: any) {
  ...
  app.post("/api/git/push", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Local workspace updates pushed to remote successfully.",
        output: stdout || stderr,
      });
    } catch (error: any) {
  ...
  app.post("/api/git/health-check", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Integrity check done.",
        output: stdout || stderr,
      });
    } catch (error: any) {
      console.error("Git Health API Error:", error);
      res.status(500).json({
        error: error.message || "Failed workspace integrity check.",
        output: error.stdout || error.stderr || "",
      });
    }
  });
  ...
  app.get("/api/staging/verify-all", async (req, res) => {
    const report: any = {
      ...
      environment: {
        GEMINI_API_KEY: process.env.GEMINI_API_KEY
          ? "CONFIGURED (MASKED)"
          : "NOT_FOUND",
        ALLDATA_API_KEY: process.env.ALLDATA_API_KEY
          ? "CONFIGURED (MASKED)"
          : "NOT_FOUND",
        NEXPART_API_KEY: process.env.NEXPART_API_KEY
          ? "CONFIGURED (MASKED)"
          : "NOT_FOUND",
        MELI_API_KEY: process.env.MELI_API_KEY
          ? "CONFIGURED (MASKED)"
          : "NOT_FOUND",
    ...
    try {
      ...
      const vpicRes = await fetch(
        "https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVin/5UM?format=json",
        { signal: controller.signal },
      );
      clearTimeout(id);
    ...
    try {
      ...
      const geminiRes = await fetch(
        "https://generativedecoding.googleapis.com",
        { signal: controller.signal },
      );
      clearTimeout(id);
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      const { message, image, history, systemInstruction, customApiKey } =
        req.body;
      ...
      if (!apiKey) {
        return res.status(400).json({
          error:
            "Gemini API Key is missing. Provide it in the API Keys screen or set GEMINI_API_KEY in the environment.",
        });
      }
      ...
      try {
        ...
        console.warn(
          "Active Gemini query failed, routing to local staging diagnostics analyzer:",
          geminiError,
        );
        apiResponseText = generateIntelligentFallback(
          message || "",
          systemInstruction || "",
        );
      }
